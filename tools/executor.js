...
import { setPositionInstruction, setDeployBaseline } from "../state.js";
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Fire-and-forget: record USD price + TVL at deploy as dump detection baselines
        ...
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, reason: result.close_reason ?? args.reason ?? null }).catch(() => {});
